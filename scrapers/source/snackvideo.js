...
class SnackVideo {
  search = async function search(q) {
    return new Promise(async (resolve, reject) => {
      await axios
        .get("https://www.snackvideo.com/discover/" + q)
        .then((a) => {
          ...
          let result = json
            ...
            .map((a) => ({
              ...
              author: {
                ...
                stats: {
                  likes:
                    a.creator.mainEntity.interactionStatistic[0]
                      .userInteractionCount,
                  followers:
                    a.creator.mainEntity.interactionStatistic[1]
                      .userInteractionCount,
                },
              },
              url: a.url,
  ...
  download = async function download(url) {
    return new Promise(async (resolve, reject) => {
      await axios.get(url).then((a) => {
        ...
        result.metadata.watch =
          json.interactionStatistic[0].userInteractionCount;
        result.metadata.likes =
          json.interactionStatistic[1].userInteractionCount;
        result.metadata.share =
          json.interactionStatistic[2].userInteractionCount;
