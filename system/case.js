...
// - Tambahkan ( // category: "nama_category" ) agar tidak termasuk ke menu case (uncategorized)
//============================
...
const { writeExif } = require("@library/sticker");
    ...
    case "cases":
      {
        ...
        if (text.includes("--add")) {
          if (!m.quoted)
            return m.reply(
              "> *⚠️ Balas dengan fitur case yang ingin disimpan*.",
            );
          let status = Case.add(m.quoted.body);
          m.reply(
            status
              ? "> *✅ Berhasil menambahkan case baru!*"
              : "> *❌ Gagal menambahkan case baru*.",
          );
        } else if (text.includes("--delete")) {
          let input = text.replace("--delete", "").trim();
          if (!input)
            return m.reply("> *⚠️ Masukkan nama case yang ingin dihapus*!");
          let status = Case.delete(input);
          m.reply(
            status
              ? `> *✅ Berhasil menghapus case: ${input}!*`
              : `> *❌ Case ${input} tidak ditemukan. Periksa daftar case yang tersedia*.`,
          );
        } else if (text.includes("--get")) {
          let input = text.replace("--get", "").trim();
          if (!input)
            return m.reply("> *⚠️ Masukkan nama case yang ingin diambil*!");
          if (!Case.list().includes(input))
            return m.reply("> *❌ Case tidak ditemukan!*");
    ...
    case "daftar":
      {
        ...
        let list = Object.values(db.list().user).find(
    ...
    case "jadwalsholat":
      {
        ...
        try {
          const { data } = await axios.get(
            `https://jadwal-sholat.tirto.id/kota-${kota}`,
          );
          const $ = cheerio.load(data);

          const jadwal = $("tr.currDate td")
            .map((i, el) => $(el).text())
            .get();
...
╰────────────•`;
            ...
            await m.reply(
              "❌ Jadwal sholat tidak ditemukan. Pastikan nama kota sesuai.",
            );
          }
        } catch (error) {
    ...
    case "s":
      {
        ...
        if (/image|video|webp/.test(quoted.msg.mimetype)) {
          ...
          m.reply(
            "> *📸 Balas dengan foto atau video untuk dijadikan sticker*.",
          );
        }
      }
      break;
          ...
          return new Promise((resolve, reject) => {
            ...
            axios
              ...
              .then((data) => {
                ...
                const hasil = [];
                $("#primary > div > div > ul > li > article").each(
                  function (a, b) {
                    hasil.push({
    ...
    case "hentaisearch": {
      ...
      function hentaisearch(query) {
        return new Promise((resolve, reject) => {
          axios
            .get(`https://sfmcompile.club/?s=${encodeURIComponent(query)}`)
            .then((data) => {
              const $ = cheerio.load(data.data);
              const hasil = [];
    ...
    case "husbando":
      {
        ...
        try {
          let response = await (
            await fetch("https://nekos.best/api/v2/husbando")
          ).json();
    ...
    case "kitsune":
      {
        ...
        try {
          let response = await (
            await fetch("https://nekos.best/api/v2/kitsune")
          ).json();
    ...
    case "waifu":
      {
        ...
        try {
          let response = await (
            await fetch("https://nekos.best/api/v2/waifu")
          ).json();
...
fs.watchFile(file, () => {
  fs.unwatchFile(file);
  console.log("- Terjadi perubahan pada files system/case.js");
