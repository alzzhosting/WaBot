  ...
  async run(m, { text, sock }) {
    ...
    try {
      if (text.startsWith("--search")) {
        ...
        if (data.length === 0) {
          return m.reply(
            "Anime tidak ditemukan. Coba lagi dengan kata kunci lain.",
          );
        }
...
Gunakan perintah \`animecorner --detail <link>\` untuk melihat detail lebih lanjut.
      ...
      if (text.startsWith("--detail")) {
        ...
        if (!url)
          return m.reply(
            "Link tidak ditemukan. Harap masukkan link anime yang valid.",
          );
...
const CorNer = {
  search: async (text) => {
    try {
      ...
      $("ul.penci-wrapper-data.penci-grid li.list-post").each(
        (index, element) => {
          ...
          const imageContainer = $(element).find(
            ".thumbnail div.acwp-image-placeholder-container",
          );
          let image =
            ...
            imageContainer
              ...
              ?.replace(/url\'?|\'?/g, "")
              .trim(); // Memperbaiki pengambilan gambar dengan regex
